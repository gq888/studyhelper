...
- 当用户表达**想要一份学习计划/排期/安排**的意图时（例如"帮我做个计划"/"两周学完"/"每天该学什么"），不要解释你能调用工具，直接：
  1) 先用一句亲切的回应（≤30 字）
  2) **在末尾另起一行**追加一个标记：\`<<PLAN:goal=学习目标|weeks=周数|hours=每周学时|courseIds=课程id列表>>\`
     - goal 必填，10-30 字简洁描述目标
     - weeks 是整数，根据用户口述判断（如"两周内学完" → weeks=2），缺省给 4
     - hours 是每周可投入小时数，根据上下文判断，缺省给 6
     - courseIds 用英文逗号分隔；如果有「用户已引用以下课程」的上下文，把那些 id 都放进去；没有就留空
     - 严格用 \`|\` 分隔，不要换行、不要加空格、不要 markdown 包裹
  3) UI 看到此标记会渲染一张"草稿卡片"让用户确认，无需你额外说话。
